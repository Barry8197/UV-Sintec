...
def write_to_database(summarydata , product_code):
    table = []
    Date , Time = date_time()
    for item in summarydata :
        ...
        table_temp.append(product_code)
        for data in item :
            if type(data) == str :
                for column in data.split('-') :
                    table_temp.append(column)
            else :
    ...
    for item in table :
        for i in range(len(item)):
            if i == 6 :
                ...
                item[i] = str(item[i]) + ' W/m^2'

    engine = create_engine("postgresql://postgres:uv-sintec@localhost:5432/LED_Measurements") 
    for item in table :
        df1 = pd.DataFrame([item] , columns = ["date" , "time", "product_code", "label_wavelength" , "current" 
                                               , "exposure_time" , "boxcar" , "scans" , "background" , "device_number"
                                               , "peak_wavelength" , "fwhm", "integrated_irradiance"])
        ...
        if len(index) > 1:
            check = False
            print("The LED at %s has been identified as a duplicate" % wavelength)
            print("")
            while check == False :
...
def Database_Search() :
    ...
    def print_database(product_code , wavelength , device_number):
        ...
        filtered2 = df["label_wavelength"].str.contains(wavelength)
    ...
    def menu_adjust(wavelength , product_code):
        filtered = df["product_code"].str.contains(product_code)
        wavelengthW.options = update_menu(df[filtered]["label_wavelength"])
        filtered2 = df[filtered]["label_wavelength"].str.contains(wavelength)
    ...
    wavelengthW = widgets.Dropdown(
        options =  update_menu(df[filter1]["label_wavelength"]),
    ...
    filter2 = df[filter1]["label_wavelength"].str.contains(init2)
